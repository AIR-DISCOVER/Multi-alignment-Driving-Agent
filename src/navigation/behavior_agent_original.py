...
class BehaviorAgent(BasicAgent):
    ...
    def _update_information(self):
        ...
        if self._direction == RoadOption.CHANGELANELEFT or self._direction == RoadOption.CHANGELANERIGHT:
    ...
    def run_step(self, debug=False):
        ...
        vehicle_list = self._world.get_actors().filter("*vehicle*")
        walker_list = self._world.get_actors().filter("*walker.pedestrian*")
        state, target, distance, angle, orientation = self._pedestrian_obstacle_detected(walker_list)
        if state:
            print(target, distance, angle, orientation)
            stop = input("pedestrians nearby, input to continue: ")
        ...
        lane_end_wpt = ego_vehicle_wp.next_until_lane_end(1)[-1]
        self.set_destination(lane_end_wpt.transform.location)
